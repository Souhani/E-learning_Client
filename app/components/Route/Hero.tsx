  ...
  return (
     ...
     <div className='text-black dark:text-white 800px:flex gap-4 items-center mx-8 my-1'>
      ...
      <div className='flex-1 text-center font-Poppins'>
            <motion.h1 variants={H1Variants} initial="initial" animate="animate" className='text-[2.5rem] font-[700] my-2'>{data?.layout?.banner?.title}</motion.h1>
            <motion.p variants={TextVariants} initial="initial" animate="animate" className='text-[1rem]'>{data?.layout?.banner?.subTitle}</motion.p>
            <div className='flex w-full m-2 r-10 '>
              <input  className='px-2 py-4 w-[90%] rounded-l-md dark:text-black bg-gray-300 dark:bg-gray-300 placeholder-gray-900 focus:outline-none dark:focus:border-[#6a6af0]  focus:border-[#9F70FD] focus:border-[1px]'
                    placeholder='Search Courses...'
                    value={search}
                    onChange={(e) => setSearch(e.target.value)}/>
              <div className='dark:bg-[#6a6af0] bg-[#9F70FD] px-2 py-4 w-[10%] flex items-center justify-center rounded-r-md cursor-pointer'
